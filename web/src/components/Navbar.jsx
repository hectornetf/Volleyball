...
export default function Navbar({ activeTab, setActiveTab }) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0b0f1a]/90 backdrop-blur-md border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex min-w-0 items-center justify-between h-16">
          ...
          <div className="flex min-w-0 flex-1 items-center space-x-3">
            <div className="flex w-10 h-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-tr from-emerald-500 to-cyan-500 shadow-lg shadow-emerald-500/20">
              <span className="text-xl">🏐</span>
            </div>
            <div className="min-w-0">
              <div className="flex min-w-0 items-center space-x-2">
                <span className="truncate text-lg font-black tracking-tight text-white sm:text-xl">VoleizinDosCria</span>
              ...
              <div className="flex min-w-0 items-center space-x-1.5 text-xs text-slate-400">
                <ShieldCheck className="w-3.5 h-3.5 shrink-0 text-cyan-400" />
                <span className="min-w-0 max-w-[100px] truncate font-mono font-bold text-cyan-400 sm:max-w-[160px]">{activeGroupId}</span>
                <button onClick={copiarCodigo} title="Copiar código do grupo" className="shrink-0 p-1 text-slate-500 hover:text-cyan-400">
                  <Copy className="w-3.5 h-3.5" />
                </button>
                <button onClick={compartilharCodigo} title="Compartilhar código no WhatsApp" className="shrink-0 p-1 text-slate-500 hover:text-emerald-400">
      ...
      <div className="md:hidden flex items-center gap-1 overflow-x-auto bg-slate-900/95 border-t border-slate-800 px-2 py-2">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`flex shrink-0 flex-col items-center whitespace-nowrap py-1 px-2 rounded-lg text-[10px] font-bold transition-all ${isActive
